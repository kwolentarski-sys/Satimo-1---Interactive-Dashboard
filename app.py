...
# Updated: Toggle between validation types including the new Chamber Comparison
...
def load_and_clean_data(file_name, is_comparison=False):
    ...
    try:
        ...
        # Keywords to identify start of a data block
            ...
            if start_row is not None:
                if not is_comparison:
                    ...
                    for _, row in data_cols.iterrows():
                        ...
                        if current_unit:
                            try:
                                freq, ref_eff, date_eff = float(val_id), float(val_ref), float(val_meas)
                                all_parsed_data.append({'Dipole': current_unit, 'Date_Label': current_date, 'Frequency (MHz)': freq, 'Reference Efficiency (dB)': ref_eff, 'Date Efficiency (dB)': date_eff})
                            except ValueError: pass
                else:
                    # Comparison 2-column format (Frequency, Efficiency)
                    chamber_name = str(df_raw.iloc[start_row+1, c]).strip()
                    unit_name = str(df_raw.iloc[0, 0]).split(':')[-1].strip() # Get "Proxicast #4"
...
if df is not None and not df.empty:
    if is_comp:
        # For comparison, Satimo 1 is reference. User selects the chamber to compare.
        units = df['Dipole'].unique()
        selected_unit = st.sidebar.selectbox("Select Dipole:", units)
        chambers = [ch for ch in df['Chamber'].unique() if ch != "Satimo 1"]
        selected_chamber = st.sidebar.selectbox("Compare Satimo 1 Against:", chambers)
        
        # Build a "Yearly-style" dataframe for the calculations/plot logic
        ref_df = df[(df['Dipole'] == selected_unit) & (df['Chamber'] == "Satimo 1")].rename(columns={'Efficiency': 'Reference Efficiency (dB)'})
        meas_df = df[(df['Dipole'] == selected_unit) & (df['Chamber'] == selected_chamber)].rename(columns={'Efficiency': 'Date Efficiency (dB)'})
        subset = pd.merge(ref_df[['Frequency (MHz)', 'Reference Efficiency (dB)']], meas_df[['Frequency (MHz)', 'Date Efficiency (dB)']], on='Frequency (MHz)')
        date_label = selected_chamber
    ...
    max_diff_idx, max_val = subset['Abs_Diff'].idxmax(), subset['Abs_Diff'].max()
    ...
    st.write(f"**Maximum Difference From {'Satimo 1' if is_comp else 'Reference NIST'}:** {max_val:.2f} dB at {max_freq} MHz")
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=subset['Frequency (MHz)'], y=subset['Reference Efficiency (dB)'], mode='lines+markers', name=f"<b>{'Reference Data - Satimo 1' if is_comp else 'Reference Data - NIST'}</b>&nbsp;&nbsp;&nbsp;", line=dict(color='red', width=3, dash='dash')))
    fig.add_trace(go.Scatter(x=subset['Frequency (MHz)'], y=subset['Date Efficiency (dB)'], mode='lines+markers', name=f'<b>{date_label}</b>', line=dict(color='#022af2', width=3)))
